...
class MyComponent extends React.Component {
  ...
  responseLinkedin(response) {
    ...
    let user = {
      ...
      Api.endpoints.signUp('seeker', user)
          .then(result =>{
          ...
          if(error.response.data.email && error.response.data.email[0].includes(testText)){
              console.log("your username and password.....", {username: user.email, password: user.password})

              Api.endpoints.signIn({username: user.email, password: user.password})
                  .then(result =>{
                      console.log("internal sever SignIn access tokem,", {result});
                  }).catch(error => {
                      console.log("internal sever coulndt veriy your account ", {error})
              })
          }else{
